...
from buttons import Buttons,create_checkbox,handle_checkbox
...
def set_resource_location(relative_path, relative_path2):
    ...
    resource_path_edit,resource_path_delete = os.path.join(base_path, relative_path),os.path.join(base_path, relative_path2)
...
def generate_window(mode, command, root, button_list, main_cmd_frame=None):
   ...
   command_cooldown = StringVar()
   command_alias = StringVar()

   window = Toplevel(root, bg=dark_theme.bg)
   ...
   if mode == "Add":
      ...
      entry_cooldown = Entry(window, textvariable=command_cooldown, bg=dark_theme.entry_bg, fg=dark_theme.fg, font=("Consolas", 12, "bold"))
      entry_cooldown.grid(row=2, column=1, sticky="w")
      ...
      btn_save = Button(window, text="Save", bg="#5DF15D", height=2, width=25, activebackground="#86FF70", font=("Consolas", 12, "bold"), command=lambda: update_command('Add', None, root, window, button_list, command_name.get(), command_response.get(), command_cooldown.get(), { "Broadcaster": broadcaster_permission.get(), "Moderators": moderators_permission.get(), "Everyone": everyone_permission.get() }, command_alias.get(), main_cmd_frame)).grid(row=6, column=0, padx=(300,15), pady=20, columnspan=2, sticky="w")
      ...
      Label(window, text="Cooldown(seconds): ", bg=dark_theme.bg, fg=dark_theme.fg, font=("Consolas", 14, "bold")).grid(row=2, column=0, padx=(25,10), pady=20, sticky="w")
      entry_cooldown = Entry(window, textvariable=command_cooldown, bg=dark_theme.entry_bg, fg=dark_theme.fg, font=("Consolas", 12, "bold"))
      entry_cooldown.insert(0, f"{command.cooldown/1000:.0f}")
      entry_cooldown.grid(row=2, column=1, sticky="w")
      ...
      print(broadcaster_permission.get())
      print(moderators_permission.get())
      print(everyone_permission.get())
      Label(window, text="Permissions: ", bg=dark_theme.bg, fg=dark_theme.fg, font=("Consolas", 14, "bold")).grid(row=3, column=0, padx=(25,10), pady=20, sticky="w")
      checkbox_frame.grid(row=3, column=1, sticky="nw")
      checkbox_broadcaster = create_checkbox(checkbox_frame, text=f"{'Broadcaster' :<11}", variable=broadcaster_permission, command=None)
      checkbox_moderators = create_checkbox(checkbox_frame, text=f"{'Moderators' :<11}", variable=moderators_permission,  command=None)
...
def update_command(mode, old_command, root, window, button_list, new_name, new_response, new_cooldown, new_permission, new_alias, main_cmd_frame=None):
   ...
   if mode == "Add":      
      if cooldown_match:
         ...
         messagebox.showerror('Invalid Command', 'Error: Please enter a cooldown', parent=window)
         return
      ...
      if (cmd_match):
         ...
         if result.inserted_id:
            ...
            print(main_cmd_frame)
            print("Row count watup ")
            print(row)
            ...
            button_list.append(Buttons(new_command.name, label_command, label_text, btn_edit, btn_delete))
